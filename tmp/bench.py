"""Benchmark allreduce modes at P=2,4,8. ALLREDUCE_COUNT=32768 (128KB/rank)."""
...
W,R=10,30
KB=ALLREDUCE_COUNT*4//1024

for P in [2,4,8]:
    devs=list(range(P))
    modes=["twophase","ring","bidirectional_ring"]
    if P==2: modes.append("ibing")
    print(f"\n=== P={P} ({KB}KB/rank) devices={devs} ===")
    res={}
    for m in modes:
        ...
        try:
            ...
            print(f"avg={avg:.0f} p50={p50:.0f} ok={'Y' if ok else 'N'}")
            res[m]=(avg,p50)
        except Exception as e: print(f"FAILED: {e}")
    if "ring" in res:
        ra=res["ring"][0]
        print(f"  {'Mode':22s} {'avg(us)':>10s} {'vs ring':>10s}")
        for m in modes:
            if m in res:
                a,_=res[m]; d=f"{(a-ra)/ra*100:+.1f}%" if m!="ring" else "(baseline)"
                print(f"  {m:22s} {a:10.0f} {d:>10s}")
print("\nDone")
